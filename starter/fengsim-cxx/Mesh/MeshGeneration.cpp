...
void MeshModule::MeshGeneration(TopoDS_Shape* S,
                                double        size,
                                int           refine_level,
                                QString       path)
{
    ...
    GM->mesh(3);
    // refine mesh
    for (int i = 0; i < refine_level; i++) GM->refineMesh(2);

    std::cout << (path + QString("/data/mesh/fengsim_mesh.vtk")).toStdString()
              << std::endl;
...
void MeshModule::MeasureModel(int id)
{
    ...
    ofstream            out;
    is.open(
        std::string("/home/jiping/FENGSim/"
                    "build-FENGSim-Desktop_Qt_5_10_1_GCC_64bit-Release/FENGSimDT.mesh")
            .c_str());
    // set dim
    int       n1  = 0;
    int       n2  = 0;
    ...
    char      L[len];
    // out .geo file
    out.open(std::string("/home/jiping/FENGSim/FENGSim/Measure/data/cloud_Model.vtk")
                 .c_str(),
             ios::app);
    // out << "POINTS" << endl;
    // vertices
    ...
    if (mesh_dim == 2) {
        ...
        for (int i = 0; i < n3; i++) {
            ...
            // z[2] - 1 << endl;
            if (z[3] == id) {
                // std::cout << z[0] << " " << z[1] << " " << z[2] << " " << z[3] <<
                // endl;
                if (!IsInclude(pp, points[(z[0] - 1) * 3], points[(z[0] - 1) * 3 + 1],
                               points[(z[0] - 1) * 3 + 2])) {
                    pp.push_back(points[(z[0] - 1) * 3]);
                    pp.push_back(points[(z[0] - 1) * 3 + 1]);
                    pp.push_back(points[(z[0] - 1) * 3 + 2]);
                    out << points[(z[0] - 1) * 3] << " " << points[(z[0] - 1) * 3 + 1]
                        << " " << points[(z[0] - 1) * 3 + 2] << endl;
                }
                if (!IsInclude(pp, points[(z[1] - 1) * 3], points[(z[1] - 1) * 3 + 1],
                               points[(z[1] - 1) * 3 + 2])) {
                    pp.push_back(points[(z[1] - 1) * 3]);
                    pp.push_back(points[(z[1] - 1) * 3 + 1]);
                    pp.push_back(points[(z[1] - 1) * 3 + 2]);
                    out << points[(z[1] - 1) * 3] << " " << points[(z[1] - 1) * 3 + 1]
                        << " " << points[(z[1] - 1) * 3 + 2] << endl;
                }
                if (!IsInclude(pp, points[(z[2] - 1) * 3], points[(z[2] - 1) * 3 + 1],
                               points[(z[2] - 1) * 3 + 2])) {
                    pp.push_back(points[(z[2] - 1) * 3]);
                    pp.push_back(points[(z[2] - 1) * 3 + 1]);
                    pp.push_back(points[(z[2] - 1) * 3 + 2]);
                    out << points[(z[2] - 1) * 3] << " " << points[(z[2] - 1) * 3 + 1]
                        << " " << points[(z[2] - 1) * 3 + 2] << endl;
                }
            }
        }
    }
...
void MeshModule::FileFormatCliToVTK(QString filename)
{
    ...
    char      L[len];
    for (int i = 0; i < 7; i++) is.getline(L, len);
    double height;
    ...
    int n = 0;
    for (int i = 0; i < slices.size(); i++) n += slices.SlicesNum(i);
    ...
    for (int i = 0; i < slices.size(); i++) {
        for (int j = 0; j < slices.SlicesNum(i); j++) {
            out << slices.X(i, j) << " " << slices.Y(i, j) << " "
                << slices.SliceHeight(i) << endl;
        }
    }
    out << "POLYGONS " + QString::number(slices.size()).toStdString() + " " +
               QString::number(slices.size() + n).toStdString()
        << endl;
    int m = 0;
    for (int i = 0; i < slices.size(); i++) {
...
void MeshModule::FileFormatMeshToVTK(QString Ain, QString Aout)
{
    ...
    const int len = 500;
    char      L[len];
    for (int i = 0; i < 4; i++) is.getline(L, len);
    int vn;
    sscanf(L, "%d", &vn);
    ...
    int cn;
    sscanf(L, "%d", &cn);
    out << "CELLS " + QString::number(cn).toStdString() + " " +
               QString::number(cn * 5).toStdString()
        << endl;
...
void MeshModule::VoxelMeshGeneration()
{
    ...
    ofstream out;
    out.open(
        std::string("/home/jiping/FENGSim/FENGSim/data/am_voxel_mesh.vtk").c_str());
    out << "# vtk DataFile Version 2.0" << endl;
    ...
    for (int j = 0; j < voxel_mesh.size() - 1; j++) {
        for (int i = 0; i < voxel_mesh[j].size(); i++) {
            ...
            //            out << x - voxel_h/2 << " " << y - voxel_h/2 << " " <<
            //            slices.SliceHeight(j) << endl; out << x + voxel_h/2 << " " <<
            //            y - voxel_h/2 << " " << slices.SliceHeight(j) << endl; out <<
            //            x + voxel_h/2 << " " << y + voxel_h/2 << " " <<
            //            slices.SliceHeight(j) << endl; out << x - voxel_h/2 << " " <<
            //            y + voxel_h/2 << " " << slices.SliceHeight(j) << endl; out <<
            //            x << " " << y << " " << slices.SliceHeight(j) << endl;
            double a[15 * 3] = {0.00, 0.00, 0.00, 1.00, 0.00, 0.00, 1.00, 1.00, 0.00,
                                0.00, 1.00, 0.00, 0.00, 0.00, 1.00, 1.00, 0.00, 1.00,
                                1.00, 1.00, 1.00, 0.00, 1.00, 1.00, 0.50, 0.50, 0.00,
                                0.50, 0.00, 0.50, 1.00, 0.50, 0.50, 0.50, 1.00, 0.50,
                                0.00, 0.50, 0.50, 0.50, 0.50, 1.00, 0.50, 0.50, 0.50};
    ...
    out << "CELLS " + QString::number(n * 24).toStdString() + " " +
               QString::number(n * 24 * 5).toStdString()
        << endl;
    int b[24 * 4] = {14, 8,  0, 1, 14, 8,  1, 2, 14, 8,  2, 3, 14, 8,  3, 0,
                     14, 9,  0, 1, 14, 9,  1, 5, 14, 9,  5, 4, 14, 9,  4, 0,
                     14, 10, 1, 2, 14, 10, 2, 6, 14, 10, 6, 5, 14, 10, 5, 1,
                     14, 11, 2, 3, 14, 11, 3, 7, 14, 11, 7, 6, 14, 11, 6, 2,
                     14, 12, 3, 0, 14, 12, 0, 4, 14, 12, 4, 7, 14, 12, 7, 3,
                     14, 13, 4, 5, 14, 13, 5, 6, 14, 13, 6, 7, 14, 13, 7, 4};
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < 24; j++) {
            out << 4 << " " << b[0 + j * 4] + i * 15 << " " << b[1 + j * 4] + i * 15
                << " " << b[2 + j * 4] + i * 15 << " " << b[3 + j * 4] + i * 15 << endl;
        }
    }
...
void MeshModule::PathPlanning()
{
    ofstream out;
    out.open(
        std::string("/home/jiping/FENGSim/FENGSim/data/am_path_planning.vtk").c_str());
    int n = 0;
    ...
    for (int i = 0; i < voxel_mesh.size() - 1; i++) {
        for (int j = 0; j < voxel_mesh[i].size(); j++) {
            ...
            double y2 = y1;
            out << x1 << " " << y1 << " " << slices.SliceHeight(i) + voxel_h / 2
                << endl;
            out << x2 << " " << y2 << " " << slices.SliceHeight(i) + voxel_h / 2
                << endl;
        }
    }
