...
// =======================================================================
enum PrimType { any, cube, sphere, cylinder, cone, torus, vertex, line, plane };
class Primitive
{
protected:
    PrimType      T;
...
public:
    Primitive(TopoDS_Shape* _S)
        : S(_S)
    {
    }
    PrimType      type() { return T; }
    TopoDS_Shape* Value() { return S; }
...
public:
    ...
    void ShowProperties(CADDockWidget* cad_dock) { cad_dock->SetPointModule(x); }
};
// =======================================================================
...
public:
    ...
    void ShowProperties(CADDockWidget* cad_dock) { cad_dock->SetLineModule(x1, x2); }
};
// =======================================================================
...
public:
    Plane(TopoDS_Shape* S)
        : Primitive(S)
    {
    }
    void SetData(double p1 = 0,
                 double p2 = 0,
                 double p3 = 0,
                 double d1 = 0,
                 double d2 = 0,
...
public:
    ...
    void ShowProperties(CADDockWidget* cad_dock) { cad_dock->SetBoxModule(x, p, d); }
};
// =======================================================================
...
public:
    Torus(TopoDS_Shape* S)
        : Primitive(S)
    {
    }
    void SetData(double r1 = 1,
                 double r2 = 0.25,
                 double p1 = 0,
                 double p2 = 0,
                 double p3 = 0,
...
public:
    ...
    void Add(Primitive* p) { P.push_back(p); }
    void Delete(Primitive* p)
    {
        int n = Include(*(p->Value()));
        if (n != -1) P.erase(P.begin() + n);
    }
    ...
    Primitive*    operator[](int i) { return P[i]; }
    void          Clear() { P.clear(); }
    TopoDS_Shape* Union()
    {
...
public:
    ...
    Boundary(const TopoDS_Shape& _S)
    {
        ...
        type = Neumann;
        id   = 0;
        for (int i = 0; i < 3; i++) value[0] = 0;
    }
    void SetValue(double* t)
    {
        for (int i = 0; i < 3; i++) value[i] = t[i];
    }
    double*       GetValue() { return value; }
    void          SetType(BoundaryType t = Dirichlet) { type = t; }
    void          SetId(int i = 0) { id = i; }
    TopoDS_Shape* Value() { return S; }
    BoundaryType  Type() { return type; }
    int           Id() { return id; }
};
...
public:
    ...
    int       Size() { return P.size(); }
    Boundary* operator[](int i) { return P[i]; }
    void      Clear() { P.clear(); }
    void      ExportToFile()
    {
        ofstream out;
        ...
        for (int i = 0; i < P.size(); i++) {
            if (P[i]->Type() == Dirichlet)
                out << 0 << " ";
            else if (P[i]->Type() == Neumann)
                out << 1 << " ";
            for (int j = 0; j < 3; j++) {
                out << (P[i]->GetValue())[j] << " ";
            }
