        ...
        for (int i = 0; i < E.size(); ++i) {
            for (int j = 0; j < E.size(); ++j) {
                for (int q = 0; q < E.nQ(); q++) {
                    A_c(i, j) += alpha(E.QPoint(q)) * E.Derivative(q, i) *
                                 E.Derivative(q, j) * E.QWeight(q);
                }
            }
        }
    }
...
void PoissonAssemble::AssembleVector(const Vector& u_d, Vector& b)
{
    ...
    for (cell c = u_d.GetMesh().cells(); c != u_d.GetMesh().cells_end(); ++c) {
        ...
        for (int i = 0; i < E.size(); ++i) {
            for (int q = 0; q < E.nQ(); q++) {
                b_c(i) += (-1) * alpha(E.QPoint(q)) * E.Derivative(q, i) *
                          E.Derivative(q, u_d) * E.QWeight(q);
            }
        }
        // neumann
        ...
        for (int i = 0; i < c.Faces(); ++i) {
            if (!IsDirichlet(u_c.bc(i)) && u_c.bc(i) != -1) {
                ...
                for (int j = 0; j < disc.NodalPointsOnFace(c, i); ++j) {
                    ...
                    for (int q = 0; q < E.nQ(); q++) {
                        u_c(k) +=
                            E.Value(q, j) * g_N(E.QPoint(q), u_c.bc(i)) * E.QWeight(q);
                    }
                }
            }
        }
