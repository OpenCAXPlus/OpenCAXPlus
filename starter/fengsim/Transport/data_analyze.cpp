...
{
    for (int i = 0; i < a.getsize(); i++) out << a[i] << " ";
    out << std::endl;
}
...
struct TerminationCondition {
    bool operator()(double min, double max) { return abs(min - max) <= 0.000001; }
};
...
double para3 = 1;
double para4 = 1;
double myF(double x) { return para1 * x * x * x + para2 * x * x + para3 * x + para4; }

double otherF(double x) { return log(abs(x)); }
...
std::vector<double> data_analyze::analyze()
{
    ...
    double  _pu[7] = {0.0631852, 0.2319644, 0.3333230, 0.2528207,
                      0.0986461, 0.0180199, 0.0020407};
    g4array pu(_pu, 7);
    ...
    double pu4 =
        sum(pu * g4array(0, 6) * g4array(-1, 5) * g4array(-2, 4) * g4array(-3, 3));

    std::cout << pu1 << " " << pu2 << " " << pu3 << " " << pu4 << std::endl;

    /*
                                                  %诱发裂变多重性数据
                                                  pu_=[.0062555 .0611921 .2265608
       .3260637 .2588354 .0956070 .0224705 .0025946 .0005205];
    ...
    double  _pu_[9] = {.0062555, .0611921, .2265608, .3260637, .2588354,
    ...
    double  pu_4 =
        sum(pu_ * g4array(0, 8) * g4array(-1, 7) * g4array(-2, 6) * g4array(-3, 5));
    ...
    readfile(
        _A,
        "/home/jiping/software/geant4.10.06.p02/examples/basic/MPU1.2/build/LIST.dat");
    for (int i = 0; i < _A.size(); i++) std::cout << _A[i] << std::endl;
    ...
    double f4 =
        sum(M * g4array(0, 24) * g4array(-1, 23) * g4array(-2, 22) * g4array(-3, 21)) /
        sum(M);

    std::cout << C_S << " " << f1 << " " << f2 << " " << f3 << " " << f4 << std::endl;
    ...
    double b4 =
        sum(Mb * g4array(0, 24) * g4array(-1, 23) * g4array(-2, 22) * g4array(-3, 21)) /
        1e7;

    double C_D = C_S * (f1 - b1);
    double C_T = C_S * (f2 - b2 - 2 * b1 * (f1 - b1)) / 2;
    double C_Q = C_S *
                 (f3 - b3 - 3 * b1 * (f2 - b2) - 3 * b2 * (f1 - b1) +
                  6 * b1 * (f1 - b1) * (f1 - b1)) /
                 6;
    double C_P = C_S *
                 ((f4 - b4) - 8 * b3 * (f1 - b1) - 6 * b2 * (f2 - b2) +
                  24 * b1 * b2 * (f1 - b1) + 12 * b1 * b2 * (f2 - b2) -
                  24 * pow(b1, 3) * (f1 - b1)) /
                 24;
    std::cout << C_D << " " << C_T << " " << C_Q << " " << C_P << std::endl;
    ...
    std::vector<double> _a;
    readfile(_a, "/home/jiping/software/geant4.10.06.p02/examples/basic/MPU1.2/build/"
                 "LIST_emit.dat");
    for (int i = 0; i < _a.size(); i++) std::cout << _a[i] << std::endl;
    g4array a(_a);
    std::cout << a << std::endl;
    ...
    double p_a =
        (-6 * (C_T - 2 * c / (1 + c) * C_D + 2 * pow(c, 2) / pow(1 + c, 2) * C_S) *
         pu2 * (pu_1 - 1)) /
        (pow(epsi, 2) * pow(1 + c, 2) * C_S * (pu2 * pu_3 - pu3 * pu_2));
    double p_b = (2 * (C_D - c / (1 + c) * C_S)) * (pu3 * (pu_1 - 1) - 3 * pu2 * pu_2) /
                 (epsi * (1 + c) * C_S * (pu2 * pu_3 - pu3 * pu_2));
    double p_c = (6 * (C_D - c / (1 + c) * C_S)) * pu2 * pu_2 /
                     (epsi * (1 + c) * C_S * (pu2 * pu_3 - pu3 * pu_2)) -
                 1;
    para1       = 1;
    para2       = p_a;
    para3       = p_b;
    para3       = p_c;
    double from = -10000; // The solution must lie in the interval [from, to],
                          // additionally f(from) <= 0 && f(to) >= 0
    double                    to = 10000;
    std::pair<double, double> result1 =
        boost::math::tools::bisect(&myF, from, to, TerminationCondition());
    // std::pair<double, double> result2 = boost::math::tools::bisect(&otherF, 0.1, 1.1,
    // TerminationCondition());
    std::cout << result1.first << " " << result1.second << " " << myF(result1.first)
              << std::endl;
    ...
    double s_F = 2 *
                 (C_D / (epsi * (1 + c)) - c / (epsi * pow(1 + c, 2) * C_S) -
                  s_M * (s_M - 1) * pu_2 * C_S / (pu_1 - 1)) /
                 (epsi * (1 + c) * pow(s_M, 2) * pu2);
    double s_A = C_S / (s_F * epsi * (1 + c) * pu1 * s_M) - 1;
    double m   = s_F / 473;

    //     % sprintf('%.4f',Collection);
    std::cout << s_F << " " << s_M << " " << s_A << " " << epsi << " " << C_S << " "
              << C_D << " " << C_T << " " << C_Q << " " << m << " "
