                       ...
                       PointCloud<PointXYZ>&            keypoints_tgt)
{
    ...
    for (int i = 0; i < keypoints_src.size(); i++) {
        std::cout << keypoints_src.points[i].x << " ";
        std::cout << keypoints_src.points[i].y << " ";
        std::cout << keypoints_src.points[i].z << std::endl;
    }
    std::cout << std::endl;
    for (int i = 0; i < keypoints_tgt.size(); i++) {
        std::cout << keypoints_tgt.points[i].x << " ";
        std::cout << keypoints_tgt.points[i].y << " ";
        std::cout << keypoints_tgt.points[i].z << std::endl;
    }
                     ...
                     PointCloud<Normal>&              normals_tgt)
{
    ...
    savePCDFile("normals_src.pcd", s);
    savePCDFile("normals_tgt.pcd", t);
}
                           ...
                           Eigen::Matrix4f&                 transform)
{
    ...
    std::cout << std::endl
              << "all_correspondences: " << all_correspondences->size() << std::endl;
    ...
    std::cout << "sac has converged:" << scia.hasConverged()
              << "  score: " << scia.getFitnessScore() << std::endl;
    ...
    std::cout << "has converged:" << icp.hasConverged()
              ...
              << " fitness epsilon: " << icp.getEuclideanFitnessEpsilon()
    ...
    std::ifstream is(
        "/home/jiping/FENGSim/build-FENGSim-Desktop_Qt_5_12_10_GCC_64bit-Debug/data/"
        "meas/fengsim_meas_scene_mesh_points.vtk");
    std::ofstream out(
        "/home/jiping/FENGSim/build-FENGSim-Desktop_Qt_5_12_10_GCC_64bit-Debug/data/"
        "meas/fengsim_meas_scene_mesh_points_trans.vtk");
    const int len = 256;
    char      L[len];
    while (is.getline(L, len)) {
        ...
        out << v2[0] << " " << v2[1] << " " << v2[2] << std::endl;
    }
    is.close();
    out.close();

    // dock->ui->textEdit->append(QString("has converged:") +
    // QString::number(icp.hasConverged()) + QString("\r\n"));
    // dock->ui->textEdit->append(QString("score:") +
    // QString::number(icp.getFitnessScore()) + QString("\r\n"));
    // dock->ui->textEdit->append(QString("fitness epsilon: ") +
    // QString::number(icp.getFitnessScore()) + QString("\r\n"));

    // std::ofstream out;
    out.open("./output_icp.vtk");

    for (int i = 0; i < output_icp.size(); i++) {
        out << std::setprecision(16)
            // out
            << (output_icp.points)[i].x << " " << (output_icp.points)[i].y << " "
            << (output_icp.points)[i].z << std::endl;
    }
    out.close();
    // std::cout <<"icp final tol: " << tol1 << std::endl;

    out.open("./output_icp2.vtk");

    for (int i = 0; i < tgt3.size(); i++) {
        out << std::setprecision(16)
            // out
            << (tgt3.points)[i].x << " " << (tgt3.points)[i].y << " "
...
void registration_gdt()
{
    ...
    {
    print_error ("Need one input source PCD file and one input target PCD file to
...
模型点云 if (pcl::io::loadPCDFile(argv[1], *model) < 0) { std::cerr << "Error loading
model cloud." << std::endl; return -1;
    }
...
//    cloud_mutex.lock();// for not overwriting the point cloud
    if (downSampling) {
        ...
        std::cout << "Number of points before downSampling: " << model->points.size() <<
std::endl; pcl::VoxelGrid<pcl::PointXYZ> sor; sor.setInputCloud(model);
        sor.setLeafSize(0.01, 0.01, 0.01);
        sor.filter(*model);
        std::cout << "Number of points after downSampling: " << model->points.size() <<
std::endl;
    }
    ...
    pcl::PointCloud<pcl::PointNormal>::Ptr normals(new
pcl::PointCloud<pcl::PointNormal>); pcl::search::KdTree<pcl::PointXYZ>::Ptr tree(new
pcl::search::KdTree<pcl::PointXYZ>()); ne.setInputCloud(model);
...
pcl::visualization::PCLVisualizer("Viewer")); viewer->setBackgroundColor(0, 0, 0);
        viewer->addPointCloud<pcl::PointXYZ>(model, "model");
        viewer->setPointCloudRenderingProperties(pcl::visualization::PCL_VISUALIZER_POINT_SIZE,
2, "model"); viewer->addPointCloudNormals<pcl::PointXYZ, pcl::PointNormal>(model,
normals, 10, 0.05, "normals");  // display every 1 points, and the scale of the arrow is
10 viewer->addCoordinateSystem(1.0); viewer->initCameraParameters();

        // Add point picking callback to viewer:
        struct callback_args cb_args;
        pcl::PointCloud<pcl::PointXYZ>::Ptr clicked_points_3d(new
pcl::PointCloud<pcl::PointXYZ>); cb_args.clicked_points_3d = clicked_points_3d;
