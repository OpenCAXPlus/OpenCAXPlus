...
void OCCWidget::Initialize()
{
    ...
    // x window
    // graphic driver is a pencil and qt widget is a blackboard. occ build a connection
    // between them. V3d_Viewer is the pencil and V3d_View is the blackboard.
    ...
    Viewer->SetLightOn();
    //   Handle(V3d_AmbientLight) aLight1 = new V3d_AmbientLight (Viewer,
    //   Quantity_NOC_GRAY50); Handle(V3d_DirectionalLight) aLight2 = new
    //   V3d_DirectionalLight (Viewer, V3d_YposZpos,Quantity_NOC_GRAY90,true);
    //   View->SetBackgroundColor(Quantity_NOC_BLACK);
    //   Viewer->SetLightOff(aLight2);
    //   View->MustBeResized();
    //   View->TriedronDisplay(Aspect_TOTP_LEFT_LOWER, Quantity_NOC_GOLD, 0.1,
    //   V3d_ZBUFFER); View->Camera()->SetProjectionType
    ...
    Handle(Prs3d_LineAspect) theAspect =
        new Prs3d_LineAspect(Quantity_NOC_BLACK, Aspect_TOL_SOLID, 2);
...
void OCCWidget::Erase2(Handle(AIS_Shape) S) { Context->Erase(S); }
// =======================================================================
//
...
void OCCWidget::SelectBoundary()
{
    ...
    if (Context->NbSelected() == 0) {
        ...
        for (Context->InitCurrent(); Context->MoreCurrent(); Context->NextCurrent()) {
            currentBndObj = new TopoDS_Shape(
                Handle(AIS_Shape)::DownCast(Context->Current())->Shape());
...
void OCCWidget::MoveEvent(const int x, const int y) { Context->MoveTo(x, y, View); }
