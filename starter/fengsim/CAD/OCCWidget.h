...
enum SelectOjbectType { SelectBoundaryObj, SelectDomainObj };
...
public:
    void             SetSelectType(SelectOjbectType type) { select_type = type; }
    SelectOjbectType SelectType() { return select_type; }
    void             SetPrimitivesDS(Primitives* p) { prims = p; }

public:
    void SetTableWidget(QTableWidget* t) { table_prop = t; }
...
public:
    void SetCurrentBoundaryObj(TopoDS_Shape*& S) { S = currentBndObj; }
    void SetBoundarObjs(Boundaries* bnd) { bndObjs = bnd; }
    void SetComboBox(QComboBox* box) { comboBox1 = box; }
    void SetSpinBox(QSpinBox* box) { spinBox1 = box; }
    void Select();
    void SelectBoundary();
    void SelectEdge();
    void SelectFace();
    ...
    void ClearCurrents() { Context->ClearCurrents(true); }
    void Clear() { Context->RemoveAll(true); }
    void RemoveAISObject(Handle(AIS_InteractiveObject) obj);
    void Remove();
    void Plot(const TopoDS_Shape* S);
    void Plot(const TopoDS_Shape& S);
    void Load(const TopoDS_Shape& S);
    void DisplayAll() { Context->DisplayAll(true); }
    void MoveTo(const Standard_Integer theXPix,
                const Standard_Integer theYPix,
                const Graphic3d_Mat4d  aProj,
                const Graphic3d_Mat4d  anOrient,
                const Standard_Integer aWidth,
                const Standard_Integer aHeight,
                const Standard_Boolean theToRedrawOnUpdate = Standard_True)
    {
        // Context->MoveTo(theXPix, theYPix, aProj, anOrient, aWidth, aHeight,
        // theToRedrawOnUpdate);
    }
...
public:
    void SetMachiningModule(bool t = false) { machining_module_checked = t; }
...
public:
    ...
    Handle(V3d_View) view() { return View; }
    WId WinId;
    WId GetWinId() { return WinId; }
