...
public:
    ...
    {
        return abs(x1 * y2 * 1 + x2 * y3 * 1 + x3 * y1 * 1 - x1 * y3 * 1 - x2 * y1 * 1 -
                   x3 * y2 * 1);
    }
...
public:
    MeshModule() {}
    void MeshGeneration(TopoDS_Shape* S,
                        double        size         = 0.5,
                        int           refine_level = 0,
                        QString       path         = "");
    void FileFormat();
    void MeasureModel(QString path,
                      QString filename = QString("/data/meas/fengsim_meas_target.vtk"));
    void MeasureModel(int id);
    bool IsInclude(std::vector<double> p, double x, double y, double z)
    {
        int n = p.size() / 3;
        for (int i = 0; i < n; i++) {
            if (p[i * 3] == x && p[i * 3 + 1] == y && p[i * 3 + 2] == z) return true;
        }
        return false;
    }
    ...
    void ClearSlices() { slices.Clear(); }
    int  voxel_points()
    {
        int n = 0;
        for (int i = 0; i < voxel_mesh.size() - 1; i++) n += voxel_mesh[i].size();
        return n;
    }
