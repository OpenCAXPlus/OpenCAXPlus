                  ...
                  Wonton::Point<D>*             pmax)
{
    ...
    *pmax = coords[0];
    for (auto pcoord : coords) {
        for (int d = 0; d < D; d++) {
            if (pcoord[d] < (*pmin)[d]) (*pmin)[d] = pcoord[d];
            if (pcoord[d] > (*pmax)[d]) (*pmax)[d] = pcoord[d];
        }
    }
}

template <int D, class CoordSys = Wonton::CartesianCoordinates>
bool intersect_boxes(Wonton::Point<D>     min1,
                     Wonton::Point<D>     max1,
                     Wonton::Point<D>     min2,
                     Wonton::Point<D>     max2,
                     std::vector<double>* xsect_moments)
{
    ...
    double vol = 1.0;
    for (int d = 0; d < D; d++) vol *= intmax[d] - intmin[d];
    ...
    for (int d = 0; d < D; d++) xsect_moments->push_back(moments[d]);

    return true;
}

template <int D, class CoordSys = Wonton::CartesianCoordinates>
void intersection_moments(
    std::vector<Wonton::Point<D>>              cell_xyz,
